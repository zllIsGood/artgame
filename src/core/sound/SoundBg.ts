...
class SoundBg extends BaseSound {
	 ...
	 * @param effectName
	 */
	public play(effectName: string, count = 1): void {
		if (this._currBg == effectName) {
			// console.log('this._currBg:', this._currBg)
			return;
		}
		// console.log('this._currBg:', this._currBg, effectName)
		this.stop();
	 ...
	 * @param sound
	 */
	private playSound(sound: egret.Sound, count = 1): void {
		this._currSound = sound;
		this._currSoundChannel = this._currSound.play(0, count);
		this.addSoundChannel(this._currSoundChannel);
	}
